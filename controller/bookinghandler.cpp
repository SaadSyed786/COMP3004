...
QString BookingHandler::bookStall(QString vendorUsername, QDate date){
    ...
    if (!vendor->isCompliant()){return "Compliance documents are incomplete or expired. Cannot book.";}
    ...
    bool booked = false;
    if (vendor->getCategory() == Vendor::Food){booked = mktDate->bookFood();}
    else{booked = mktDate->bookArtisan();}
...
QString BookingHandler::cancelStall(QString vendorUsername, QDate date){
    Vendor* vendor = mgr->findVendor(vendorUsername);
    if (!vendor) {return "Vendor not found.";}
    if (!mgr->hasBooking(vendorUsername, date)){return "No booking found for this date.";}
    ...
    mgr->sendNotification(vendorUsername,
        Notification("Booking cancelled for " + date.toString("yyyy-MM-dd"),Notification::BookingCancelled));

    // autobooking next person in the waitlist
    if (mktDate) {
        ...
        while (wl->size() > 0) {
            ...
            // skip only if vendor does not exist or already has a booking
            if (!nv || mgr->hasAnyBooking(nextVendor)) {
                mgr->sendNotification(nextVendor,
                    Notification("A " + cat + " stall opened for " + date.toString("yyyy-MM-dd")+ " but you already have a booking.", Notification::Info));
                continue;
            }
            ...
            if (ok) {
                ...
                mgr->sendNotification(nextVendor,
                    Notification("A " + cat + " stall for " + date.toString("yyyy-MM-dd")+ " was auto-booked for you from the waitlist!",Notification::BookingConfirmed));
            }
            break;
        }
